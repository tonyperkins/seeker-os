...
class JobStatus:
    ...
    # Hard user decision statuses — NEVER overridden by side-effect writes
    # (resume generation, refilter/rescore, etc.).  These represent explicit
    # user choices: skipped, applied, engaged, company_rejected, withdrawn,
    # offer_accepted, offer_declined, plus capped (system state tied to
    # per-company limits, not re-evaluated by rescore).
    #
    # reviewing and interested are NOT here — they are transient review states
    # that can be promoted (e.g. reviewing → interested by resume generation).
    #
    # rejected is NOT here: system rejections (hard filter, below threshold)
    # may legitimately change when filters/scores are re-evaluated.
    _DECISION_STATUSES = frozenset({
        SKIPPED, APPLIED, ENGAGED,
        COMPANY_REJECTED, WITHDRAWN, OFFER_ACCEPTED, OFFER_DECLINED,
        CAPPED,
    })

    # Statuses preserved through refilter/rescore — score is updated but
    # status is NOT changed.  This is _DECISION_STATUSES plus the transient
    # review states (reviewing, interested) which are user-set and must not
    # be changed by rescoring, but CAN be promoted by explicit user actions
    # like resume generation.
    _RESCORE_PRESERVED_STATUSES = _DECISION_STATUSES | frozenset({REVIEWING, INTERESTED})
