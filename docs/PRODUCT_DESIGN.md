...
```mermaid
graph TB
    Engine["Seeker OS Engine<br/>(generic — no personal values in code)<br/>discovery · filtering · scoring · dedup · resume_gen · crossref · pipeline"]
    Engine -->|"reads"| Profile["profile.yml<br/>WHO am I"]
    Engine -->|"reads"| Scoring["scoring_rubric.yml<br/>HOW to score"]
    Engine -->|"reads"| Accuracy["accuracy_rules.yml<br/>WHAT not to embellish"]

    Engine --> AIRules["AI Rules Layer (three-tier)"]
    AIRules --> IR["identity_rules.yml → WHO (positioning, anchor)"]
    AIRules --> CR["channel_rules.yml → HOW (per-channel format)"]
    AIRules --> AP["jobs.ai_policy → WHEN (per-application)"]

    Engine --> Ops["Operational Config<br/>queries.yml · filters.yml · blacklist.txt<br/>(already configurable)"]
```
