...
A legacy system used simple `source:slug:jobid` string keys. This catches
...
```mermaid
graph TB
    Start["New job arrives at Tier 1"]
    L1["Layer 1: Exact URL Hash<br/>sha256(apply_url) — UNIQUE index<br/>Catches ~70% · Cost: O(1)"]
    L2["Layer 2: Composite Key<br/>{source}:{board_token}:{job_id}<br/>Catches ~20% (cross-source) · Cost: O(1)"]
    L3["Layer 3: Content Hash<br/>md5(first 500 chars of normalized JD)<br/>Catches ~5% (reposts) · Cost: O(1)"]
    L4["Layer 4: Fuzzy Match<br/>rapidfuzz title > 90 + company > 85<br/>Catches ~5% (fuzzy) · Cost: O(n)"]
    New["NEW JOB — insert"]
    ...
    L3 -->|"not found"| L4
    L4 -->|"not found"| New
    L1 -->|"found"| Dup["Duplicate flagged"]
    L2 -->|"found"| Dup
    L3 -->|"found"| Dup
    L4 -->|"found"| Review["Flagged for review (not merged)"]
```
...
```python
# source_map is loaded from config/sources.yml (not hardcoded)
...
The key insight from a prior feasibility spike: hiring.cafe's `id` field decomposes
