...
```mermaid
graph TB
    Caller["Task Caller<br/>(resume gen, accuracy check, onboarding, etc.)"]
    Caller -->|"calls"| Router["Model Router<br/>maps task → tier → provider + model<br/>resolves per-task overrides"]
    Router -->|"routes to"| A["Provider A<br/>(anthropic)<br/>direct"]
    Router -->|"routes to"| B["Provider B<br/>(openai-compat)<br/>Kilo"]
    Router -->|"routes to"| C["Provider C<br/>(openai-compat)<br/>Ollama"]
```
...
- Uses the same client_id as the Claude CLI (shared PKCE flow)
